...
document.addEventListener('scroll', function() {
  ...
  // Función para manejar los metadatos del enlace
function handleLinkMetadata() {
  // Obtener los metadatos de la URL si existen
  const urlParams = new URLSearchParams(window.location.search);
  const metadata = urlParams.get('metadata');
  
  if (metadata) {
    // Guardar los metadatos en localStorage
    localStorage.setItem('linkMetadata', metadata);
  }
  
  // Si estamos volviendo y hay metadatos guardados
  const savedMetadata = localStorage.getItem('linkMetadata');
  if (savedMetadata && !metadata) {
    // Aplicar los metadatos guardados a la página actual
    document.title = savedMetadata;
    // También podríamos actualizar otras etiquetas meta si es necesario
  }
}

document.addEventListener('DOMContentLoaded', function() {
    // Llamar a la función de manejo de metadatos
    handleLinkMetadata();
